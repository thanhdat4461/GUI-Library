...
local sliding, editinghue, editingsatval
...
local function createOptionHolder(holderTitle, parent, parentTable, subHolder)
    ...
    if not subHolder then
        ...
        -- Credit To Tora IsMe for this draggable ui
		title.InputBegan:connect(function(input)
			if not sliding and not editinghue and not editingsatval and input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		...
		title.InputChanged:connect(function(input)
			if not sliding and not editinghue and not editingsatval and dragging and input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
				dragInput = input
			end
		end)
...
-- Unsupported, Unnecessary Functions
...
local function createSlider(option, parent)
	...
	bar.InputEnded:Connect(function(input)
		...
		if input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if not dragging and not sliding then
				tweenService:Create(fill, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(60, 60, 60)}):Play()
				tweenService:Create(circle, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 0, 0, 0), ImageColor3 = Color3.fromRGB(60, 60, 60)}):Play()
			end
		end
...
local function createList(option, parent, holder)
    ...
    round.MouseButton1Click:Connect(function()
        ...
        if ToggleDropdown then
            ...
            tweenService:Create(CloseHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 90}):Play()
            ...
            tweenService:Create(CloseHolder, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 180}):Play()
        end
    end)
...
function Library:Init()
	...
	Unnamed.Parent = game:GetService("CoreGui")
	Unnamed.Name = "Unnamed Library"
    
    for _, window in next, self.windows do
        if window.canInit and not window.init then
            window.init = true
            createOptionHolder(window.title, Unnamed, window)
            loadOptions(window)
        end
    end
end
